# main.py (Monolithic Bot: Game-focused, Quiz Polls, Advanced Word Hustle V6 - HTML Fix)
...
# Sirf 5-letter words ki list (API se fetch hogi)
# ------------------------------------------
...
async def start_hustle_game(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Game state ko bot_data mein save karega
...
async def handle_hustle_guess(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    # --- At this point, it IS a valid guess ---
    ...
    if text == secret_word:
        ...
        reply_text = (
            f"🎉 <b>CONGRATULATIONS!</b> {user.mention_html()} ne Word Hustle <b>{len(guesses_history)}</b> attempts mein jeet liya!\n\n"
    ...
    reply_text = (
        ...
        f"Attempts unlimited hain. Try again!"
    )
...
async def staggered_broadcast_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for chat_id in chat_ids:
            try:
                # Check: Agar uss chat mein Word Hustle game chal raha hai, toh quiz skip karo
...
async def timer_status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"<b>Last Broadcast:</b> <code>{last_quiz_time_str}</code>\n"
        f"<b>Cooldown ({GLOBAL_QUIZ_COOLDOWN}s):</b> <code>{cooldown_status}</code>\n\n"
        f"--- <b>Active Games</b> --- \n" 
...
async def send_quiz_after_n_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # (Yeh check karega ki message ek valid hustle guess hai ya nahi)
    # 💡 FIX: Yeh ab True return karega agar message ek guess tha
    was_hustle_guess = await handle_hustle_guess(update, context)
    
    # 💡 FIX: Agar message ek hustle guess tha, toh quiz trigger check mat karo
    if was_hustle_guess:
