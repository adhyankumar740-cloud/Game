# main.py (Monolithic Bot: Game-focused, Quiz Polls, Advanced Word Hustle)
...
HUSTLE_WORD_KEY = 'hustle_word'
HUSTLE_RUNNING_KEY = 'hustle_running'
HUSTLE_GUESSES_KEY = 'hustle_guesses' # List of (guess, feedback) tuples
HUSTLE_MAX_ATTEMPTS = 6
HUSTLE_WORD_LENGTH = 5

# Sirf 5-letter words ki list (Aap isko apni marzi se aur bada kar sakte hain)
FIVE_LETTER_WORDS = [
    "APPLE", "GREAT", "POWER", "CODED", "HELLO", 
    "WORLD", "TIGER", "HOUSE", "PLANE", "SHAPE",
    "CRANE", "TRAIN", "LIVES", "FUNNY", "DREAM",
    "BLANK", "STARE", "MOUNT", "GHOST", "FRESH",
    "AUDIO", "BRAIN", "CLICK", "DEBUG", "EMAIL",
    "PROXY", "QUERY", "VIRUS", "WRITE", "LOGIC"
]
# ------------------------------------------
...
# ======================================================================
# --- 🔠 WORD HUSTLE (WORDLE-STYLE) GAME LOGIC ---
# ======================================================================
...
def get_hustle_feedback(secret_word: str, guess: str) -> str:
    ...
    # Step 1: Green (Correct letter, Correct place)
    for i in range(HUSTLE_WORD_LENGTH):
        if guess[i] == secret_word[i]:
            result_array[i] = GREEN
            secret_counts[guess[i]] -= 1

    # Step 2: Yellow (Correct letter, Wrong place) and Black (Not in word)
    for i in range(HUSTLE_WORD_LENGTH):
        if result_array[i] is None: # Sirf non-green letters ko process karein
            if secret_counts.get(guess[i], 0) > 0:
                result_array[i] = YELLOW
                secret_counts[guess[i]] -= 1
            else:
                result_array[i] = BLACK
...
def create_game_board(guesses_history: list) -> str:
    ...
    for guess, feedback in guesses_history:
        # Har guess ko bold aur feedback ke saath dikhayenge
        board_text += f"<b>{html.escape(guess.upper())}</b>\n{feedback}\n\n"
        
    # Baaki bachi attempts ke liye empty slots
    remaining_attempts = HUSTLE_MAX_ATTEMPTS - len(guesses_history)
    empty_slot = " ".join(["⬛"] * HUSTLE_WORD_LENGTH)
    for _ in range(remaining_attempts):
        board_text += f"{empty_slot}\n"
...
async def start_hustle_game(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Agar game pehle se hi chal raha ho
    if chat_data.get(HUSTLE_RUNNING_KEY, False):
    ...
    chat_data[HUSTLE_WORD_KEY] = secret_word
    chat_data[HUSTLE_RUNNING_KEY] = True
    chat_data[HUSTLE_GUESSES_KEY] = []
    
    intro_message = (
        "<b>🎲 Word Hustle START!</b>\n\n"
        f"Ek {HUSTLE_WORD_LENGTH} letters ka shabdh chuna gaya hai. \n"
        f"Aapke paas <b>{HUSTLE_MAX_ATTEMPTS}</b> attempts hain.\n\n"
        "Har guess ke baad, yeh feedback milega:\n"
...
async def stop_hustle_game(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not chat_data.get(HUSTLE_RUNNING_KEY, False):
        await update.message.reply_text("Word Hustle abhi chal nahi raha hai.", parse_mode=constants.ParseMode.HTML)
        return

    secret_word = chat_data.get(HUSTLE_WORD_KEY, '??????')
    
    # State reset
    chat_data[HUSTLE_RUNNING_KEY] = False
    chat_data.pop(HUSTLE_WORD_KEY, None)
    chat_data.pop(HUSTLE_GUESSES_KEY, None)
...
async def handle_hustle_guess(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Har message ko Word Hustle guess ki tarah check karne ke liye"""
    # Safety check: update.message ya update.edited_message se text check karo
    ...
    # Agar game nahi chal raha hai, toh ignore
    if not chat_data.get(HUSTLE_RUNNING_KEY, False):
        return

    text = message.text.strip().upper()
    
    # 1. Length aur type check
    if len(text) != HUSTLE_WORD_LENGTH or not text.isalpha():
        # Koi reply nahi bhejenge, bas ignore kar denge taaki dusre handlers (quiz trigger) kaam kar sakein
        return
    
    # Agar guess ek command hai toh ignore
    if text.startswith('/'):
        return

    secret_word = chat_data[HUSTLE_WORD_KEY]
    guesses_history: list = chat_data.get(HUSTLE_GUESSES_KEY, [])
    
    # 2. Feedback aur history update
    feedback_emojis = get_hustle_feedback(secret_word, text)
    guesses_history.append((text, feedback_emojis))
    chat_data[HUSTLE_GUESSES_KEY] = guesses_history 

    # 3. Game Board banao
    current_board = create_game_board(guesses_history)
    
    # 4. Game State check (Jeet/Haar)
    ...
    if text == secret_word:
        ...
        new_score = current_score + 5 # Word Hustle jeetne par 5 points
        ...
        reply_text = (
            ...
            f"Aapko <b>5 points</b> mile! Total score: {new_score}\n\n"
            f"<pre>{current_board}</pre>" # Display final board
        )
        await message.reply_text(reply_text, parse_mode=constants.ParseMode.HTML)
        return

    # --- LOSE Condition ---
    if len(guesses_history) >= HUSTLE_MAX_ATTEMPTS:
        ...
        reply_text = (
            f"😔 <b>Word Hustle End!</b> Aapne <b>{HUSTLE_MAX_ATTEMPTS}</b> attempts poore kar liye hain.\n\n"
            f"Sahi shabdh tha: <b>{secret_word}</b>\n\n"
            f"Khelne ke liye shukriya! Naya game shuru karne ke liye <code>/hustle</code> type karein."
        )
        await message.reply_text(reply_text, parse_mode=constants.ParseMode.HTML)
        return
    ...
    # --- Game Continuing ---
    reply_text = (
        f"<b>Guess #{len(guesses_history)}</b> by {update.effective_user.first_name}:\n\n"
        f"<pre>{current_board}</pre>\n"
        f"Aapke paas ab <b>{HUSTLE_MAX_ATTEMPTS - len(guesses_history)}</b> attempts bache hain."
    )
    ...
    try:
        ...
        for chat_id in chat_ids:
            try:
                ...
                if active_hustle_games.get(str(chat_id), {}).get('active', False):
...
async def send_quiz_after_n_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if current_time - last_quiz_time > GLOBAL_QUIZ_COOLDOWN:
        ...
        # Cooldown chal raha hai, aur yeh ek hustle guess bhi nahi tha.
        pass
