# main.py (Monolithic Bot: Game-focused, Quiz Polls, Advanced Word Hustle V5 - API Words)
...
# 💡 REMOVED: Static FIVE_LETTER_WORDS list
# ------------------------------------------
...
async def fetch_hustle_word_from_api():
    ...
    try:
        ...
        if len(word) == HUSTLE_WORD_LENGTH:
            return word
        else:
            logger.warning(f"API returned a word with incorrect length: {word}")
...
async def timer_status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 💡 FIX: Yeh ab HUSTLE_GAME_KEY se count karega
    active_games = get_bot_value(HUSTLE_GAME_KEY, {})
