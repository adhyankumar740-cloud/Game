# main.py (Monolithic Bot: Game-focused, Quiz Polls, Advanced Word Hustle V2)
...
# HUSTLE_MAX_ATTEMPTS = 6 (REMOVED - Unlimited Attempts)
...
def get_hustle_feedback(secret_word: str, guess: str) -> str:
    ...
    # Step 2: Yellow (Correct letter, Wrong place) and Red (Not in word)
    for i in range(HUSTLE_WORD_LENGTH):
        if result_array[i] is None: # Sirf non-green letters ko process karein
            if secret_counts.get(guess[i], 0) > 0:
                ...
                result_array[i] = RED # 💡 CHANGED: Black se Red
...
def create_game_board(guesses_history: list) -> str:
    ...
    # 💡 REMOVED: Empty slots ab nahi dikhayenge, attempts unlimited hain
...
async def start_hustle_game(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # 💡 UPDATED: Intro message (Unlimited attempts, Point deduction)
    intro_message = (
...
async def handle_hustle_guess(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # --- LOSE Condition (REMOVED) ---
    
    # --- Game Continuing (WRONG GUESS) ---
    # 💡 Har galat guess par point kaato
    current_score = get_user_score(user.id)
...
async def send_quiz_after_n_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Yeh handler ab sirf new text messages ke liye hai (edited nahi)
    ...
    # Agar yeh ek valid guess hai, toh handle_hustle_guess function message reply kar dega
    ...
    if current_time - last_quiz_time > GLOBAL_QUIZ_COOLDOWN:
        # Check karo ki koi hustle game active na ho
        active_hustle_games = get_bot_value(HUSTLE_GAME_KEY, {})
        if active_hustle_games.get(str(chat_id), {}).get('active', False):
            logger.info("Quiz trigger skipped, Word Hustle is active in this chat.")
            return
